...
// @access  Private
router.post('/create-reservation-payment', [
  protect,
  body('amount').isFloat({ min: 100 }).withMessage('Deposit amount must be at least 100'),
    ...
    if (reservation) {
      ...
      try {
        ...
        const text = [
          `Hi ${reservation.name},`,
          ``,
          `Your reservation deposit has been received successfully.`,
          ``,
          `Reservation details:`,
          ...
          ``,
          `Thank you for booking with ${restaurantName}.`,
        ].join("\n");
...
router.get('/stats', protect, async (req, res) => {
  try {
    ...
    const successfulPayments = payments.items.filter(payment => payment.status === 'captured').length;
    const failedPayments = payments.items.filter(payment => payment.status === 'failed').length;
